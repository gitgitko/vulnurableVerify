...
function connectToParty() {
  ...
  partySocket.addEventListener("message", async (message) => {
    ...
    // NOTE: previously there was an `if (type === "j") { eval(...) }` block here.
    // It was removed because it allowed remote code execution via the server.
    if (type === "D") {
...
// FIX: coerce hue to a safe integer (was: `hsl(${hue}, 40%, 40%)` directly)
function getUsernameColor(hue) {
  const h = Number.isFinite(+hue) ? Math.abs(parseInt(hue, 10)) % 360 : 0;
  return `hsl(${h}, 40%, 40%)`;
}
...
function addOptPings(string, addPings, globalPingValid) {
  ...
  // FIX: escape usernames before inserting them; use function replacements
  // so "$" characters inside user[0] are not interpreted as regex backrefs.
  verifiedUsers.forEach((user) => {
    const safeName = stripHTML(user[0]);
    string = string.replace(
      "@" + user[0],
      () => `<span class="ping">@\u200b${safeName}</span>`
    );
  });
  if (globalPingValid) {
    string = string.replace(
...
function displayChatMessage(data) {
  ...
  try {
    ...
    if (data.broadcast) {
      ...
      // FIX: strip anything that isn't a valid CSS class token from neo/textneo
      const safeNeo = String(data.neo ?? "default").replace(/[^a-zA-Z0-9_-]/g, "");
      const safeTextNeo = String(data.textneo ?? "default").replace(/[^a-zA-Z0-9_-]/g, "");
      ...
      let stylingData = data.neo !== "default" ? "class='neo " + safeNeo + "' style='cursor: pointer;'" : "style='color: " + stripHTML(color) + "; cursor: pointer;'";
      ...
      outputHTML += "</span>";
      if (!data.imgURL) {
        outputHTML += "<br><span class='neo " + safeTextNeo + "'>" + strippedMsg + "</span>";
